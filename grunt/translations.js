...
function chooseNativeCountryName(countryStats, entries, country, localizedName) {
  ...
  //* Take the most popular names, allow more than one for very popular names
  const appendIf = {
    0: 0.0,
    1: 20,
    2: 30,
  ...
  for (const [_, lngStat] of Object.entries(countryStats)) {
    ...
    if(percent >= appendIf[result.length]) {
      result.push(entriesWithoutLocalizedName[lngStat.code]);
    }
  ...
  //* No usable clue for this country statistics, so compute the number of language using a name
  let namesCount = {}
